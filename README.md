...
**Overlay:** **Hover** shows country name and IP. **Local network** (localhost / private LAN) uses the **house** icon. **Geo request failed** uses a **red warning** chip; the tooltip includes a **brief** technical hint. When coordinates are available with a **flag**, **click** the flag to open **OpenStreetMap** (`mlat` / `mlon`).
