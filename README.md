...
Minimal **Chrome** and **Firefox** extension (Manifest V3): resolves the **top-level page hostname** to an IP, looks up **approximate country** using **public IP geolocation APIs** (or **only** your self-hosted endpoint when you configure one in options), and shows a **small flag** in the bottom-right of the page. **Country name and IP appear only on hover.**
...
## Extension options (self-hosted URL + public providers)
...
**Public geo providers (checkboxes):** When the **custom base URL field is empty**, country lookup uses only the **checked** public providers, in a fixed order (ipwho.is → ipinfo.io → ipapi.co → geojs.io → reallyfreegeoip.org → ip-api.com HTTP). At least one must stay checked to save. When a **custom base URL is saved**, those providers are **not** used for country lookup (only your URL is).
...
- A thin object: **`countryCode`** / **`country_code`** (two-letter ISO), with optional **`country`** / **`country_name`** / **`countryName`** for the display name; or **MaxMind-style flat JSON** where **`country` is the ISO-2 code** (e.g. `"country":"DE"`) and there is no separate `countryCode` — the UI name is then derived via `Intl` (e.g. “Germany”), or  
...
**Optional coordinates:** If the JSON (homelab or public APIs) includes valid **`latitude`** / **`longitude`** (or common aliases such as **`lat`** / **`lon`**, **`loc`** as `"lat,lng"`, or a **`location`** object), the overlay may offer **click-to-open** that position on **OpenStreetMap** in a new tab (only when both coordinates are present and valid).

**Behaviour:** If a base URL is saved, the service worker uses **only** that origin for country lookup (`GET {base}/{ip}`), with a **1 second** fetch timeout per attempt. **Public geo APIs are not called** until you clear the custom URL in options. If the custom URL field is **empty**, only the **selected** public providers are tried (see checkboxes above). On homelab failure (network, timeout, HTTP error, or unrecognised JSON), the overlay shows a **warning icon** (red “!” chip) and a **short error hint in the tooltip** (e.g. `HTTP 502`, `Timeout`); there is **no** third-party fallback and **no** backoff skip — the next navigation retries the homelab request immediately. Host resolves from the service worker run **one at a time** so cache updates from parallel tabs stay consistent.

**Overlay:** **Hover** shows country name and IP. **Local network** (localhost / private LAN) uses the **house** icon. **Geo request failed** uses a **red warning** chip; the tooltip includes a **brief** technical hint. When coordinates are available with a **flag**, **click** the flag to open **OpenStreetMap** (`mlat` / `mlon`). The chip is attached under **`<html>`** with **maximum `z-index`** so it stays above most page UI; if a client-side router **removes** it from the DOM, a **debounced** check **re-mounts** the overlay (also on **`popstate`** / **`hashchange`**).

**Resolve cache (in-memory, per browser profile):** Successful host lookups are cached in the extension service worker for **15 minutes** (`hostname` + protocol + geo settings). Failed resolves are not written to that cache. Changing options or geo-related storage clears the cache.

**Permissions:** Saving a non-empty URL triggers a **host permission** prompt for that origin (via `optional_host_permissions` patterns `http://*/*` and `https://*/*`). Clearing the custom URL removes the stored URL only (it does not revoke already granted origins).

**Storage:** If you use **“Delete extension storage”** (or similar) in `about:debugging`, that wipes **`customGeoBaseUrl`** and **`enabledPublicGeoProviders`** too — the extension will **not** call your homelab until you open **options** again and **save** (and accept the host permission prompt if shown). Provider checkboxes reset to **all enabled** when that key is missing.

**Troubleshooting:** Confirm the host permission was **allowed** when saving. Reload the extension or revisit the page — the in-memory cache is keyed by your saved base URL and provider list. **DevTools:** With a custom URL, the **service worker** Network tab usually shows only **`geoip.tma`** (and on **Chrome**, DoH `fetch` calls for hostname→IP). **Firefox** uses `browser.dns.resolve` for the tab hostname — that does **not** appear as HTTP. The **flag image** (`flagcdn.com`) is requested from the **page** context (content script), so open the **website’s** Network tab to see it, not only the extension worker. Homelab calls use **exactly** the URL you saved (`https://geoip.tma/…` or `http://…`), never a rewritten IP host. If DevTools still show `https://<LAN-IP>/…`, check that **options** do not use a literal-IP base URL and that your **server does not redirect** the browser to an IP URL. **Firefox / LibreWolf:** if `fetch` to an internal hostname fails while `browser.dns.resolve` works, fix **split-horizon / extension DNS** so the hostname request succeeds. With a custom URL set you should see **no** requests to ipwho.is / ipinfo.io etc.; if you do, the custom URL is cleared or the extension was not reloaded. If the flag stays “?” despite HTTP 200 from `geoip.tma`, check the JSON uses **`countryCode`** (two letters) or **`country_code`**, or the cloud66-style `country.iso_code` object — otherwise parsing fails and the UI shows Unknown.
...
| **`storage`** | Save the optional custom geo base URL and **which public geo providers are enabled**. |
...
- **All browsers:** The **resolved IP address** to **IP geolocation** services you enabled in options (when no custom URL is set), or **only** your custom geo URL when it is set.

If you configure a **custom geo base URL** in options, the **resolved public IP** is sent **only** to **that** origin using `GET {your base}/{ip}` as documented above. **No** public geolocation APIs are used for that lookup until you remove the URL. Those requests are made by the extension’s **service worker**, not by the web page. No separate analytics or telemetry are implemented in this repository; any logging depends on the browser and OS.
...
**All — Geolocation (only when no custom geo URL; fixed order, subset by options):** ipwho.is, ipinfo.io, ipapi.co, geojs.io, reallyfreegeoip.org, ip-api.com (HTTP). Uncheck providers you do not want to contact (e.g. blocked by Pi-hole). `NS_ERROR_CONNECTION_REFUSED` (or similar) on a host usually means a **firewall, DNS filter, or Pi-hole** is blocking that domain — a later **checked** provider in the chain may still succeed.
...
2. **No affiliation.** This project is **not** affiliated with, endorsed by, or sponsored by Google, Cloudflare, Quad9, Cisco/OpenDNS, AdGuard, ipinfo, ipwho, ipapi, GeoJS, reallyfreegeoip, ip-api, flagcdn, Mozilla, or any other third-party service.
