...
**Behaviour:** If a base URL is saved, the service worker uses **only** that origin for country lookup (`GET {base}/{ip}`). **Public geo APIs are not called** until you clear the custom URL in options. If the custom URL field is **empty**, only the **selected** public providers are tried (see checkboxes above). On homelab failure (network, timeout, HTTP error, or unrecognised JSON), the country is shown as **Unknown** (no third-party fallback). After a failed attempt, homelab is **skipped for about 5 minutes** (persisted in `storage`) so every page load does not wait on the homelab timeout; during that window the country stays **Unknown** until the window ends or you save the URL again (which clears backoff). A successful homelab response clears backoff. Host resolves from the service worker run **one at a time** so backoff updates from parallel tabs/frames cannot overwrite a success.
...
| **`storage`** | Save the optional custom geo base URL, **which public geo providers are enabled**, and homelab retry/backoff timestamps. |
